 ...
 * `username` is the in-game display name — players can type anything (spaces,
 * digits, accents, symbols, any mix), the only rule is a length bound. It's
 * enforced case-insensitively unique in the DB (migration 0002) with a
 * matching length-only CHECK (migration 0015 relaxes the old character-class
 * restriction from 0011/0012).
 */
