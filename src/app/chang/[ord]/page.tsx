...
/**
 * Per-stage game route (reached from /map). Chặng 1 is the playable
 * "Hồ sơ thất lạc" game; chặng 2–3 show a placeholder until their games are
 * built. Protected by middleware; the server resolves the signed-in user and
 * hands id + username to the client game (presence + progress tracking).
 */
export default async function ChangPage({ params }: { params: Promise<{ ord: string }> }) {
  ...
  if (ord === 1) {
    return <Chang1Game userId={user.id} username={authUser?.username ?? 'Player'} />;
  }
